...
| 模块类型 | 方案设计轨道（数据基础设施） |
| 状态 | 定稿（v1.2.0 修订：草稿降级层改 IndexedDB、恢复缺失键取已保存值、zip 导入安全校验、LRU 钉住语义） |
...
v1.1.0 起 M1 同时承载**草稿缓存文件**（`draft.json`，总规划决策 #10）：持久化存储用户当前皮肤选择与在其基础上自定义的叠加参数（未保存修改）——它是「预览 · 示例」所渲染草稿的持久化载体（防误关页面丢失、跨会话恢复），主存与皮肤库同在 OPFS（降级层为 IndexedDB，见 §8），但生命周期独立（「保存」/「放弃更改」即清除）。注意：页面运行期的未保存状态是组件工作副本（内存、易失），不是缓存文件，也不作为其降级层；契约见 §8。
...
// LRU 钉住语义（决策 1.3 补充，v1.2.0）：resolveSkin 返回的记录在被组件挂载
// 期间处于 pinned 状态（SkinBubbleChrome 挂载 pin / 卸载 unpin，按 skinId 引用
// 计数）；LRU 淘汰仅作用于未钉住条目——「组件持引用则 URL 不淘汰」由该机制
// 保证，而非依赖巧合。

// —— 草稿缓存文件（v1.1.0 新增，总规划决策 #10，详见 §8）——
interface SettingsDraft { version: 1; updatedAt: string; values: Record<string, unknown> }
interface DraftStore {
  available: boolean                        // 持久层可用性（OPFS / IndexedDB 至少其一；皆不可用 = false，页面回落即时提交，见 §8）
...
- ZIP 解压安全（导入 importPackage 与编辑器打开 openPackage 共用）：
  - 条目名仅允许 basename——含路径分隔符（`/`、`\`）或 `..` 即**整包拒绝**（防 zip-slip 路径穿越，落盘路径一律由本方拼接 `skins/<新id>/assets/<basename>`）；
  - 解压过程**逐条目流式累计**大小，超 20MB 即中止拒绝（限额在解压中执行而非解完再查，防解压炸弹）；
  - 条目数 ≤256，超出拒绝。
...
- **存储策略（与内存严格区分）**：草稿缓存文件只有两级**持久层**——主存 OPFS `<根>/draft.json`，OPFS 不可用降级 **IndexedDB**（库 `dsh-chat-focus`、store `draft`、主键 `draft`；DraftStore 本就是异步接口，封装成本约几十行）。不用 localStorage：草稿为全字段整值快照，双侧 bgImage data URI 最坏各 ~2MB（base64 后 ~2.7MB），合计 ~5.3MB 会超其典型配额，且同步大字符串写入阻塞 UI 线程（决策 #12）。到此为止。页面运行期的未保存状态只是组件工作副本（内存、易失、随刷新消失），仅服务本会话交互与「预览 · 示例」渲染——它不是缓存文件，也不作为缓存文件的降级层；
...
- **版本兼容**：`version` 前向兼容——未知键忽略、**缺失键取当前已保存设置值**（非 schema 默认值：否则旧版本草稿恢复后点「保存」，会把新增字段的已存值静默重置为默认）；解析失败视为无草稿并静默清除损坏文件。
...
| OPFS 与 IndexedDB 均不可用 | `available=false`：不产生草稿缓存文件，设置页回落逐字段即时提交（当前 v0.2.5 行为） | 横幅「更改暂存不可用，已切换为即时生效模式」 |
