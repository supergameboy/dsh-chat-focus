...
| 修订记录 | v1.1.0（2026-08-22）：两段式应用模型（决策 #10，见 §3 / §4.1 / §4.5 / §5.4）；v1.2.0（2026-08-22）：基线更新至 v0.2.5（host 0.1.1-rc.2 统一双侧 ChatBubble 管线），刷新 §1.2 代码事实、改写 M2/M5 用户侧集成、M6 决策 6.3 标记已由上游完成、§9 回退为待实施 |
...
flowchart TD
    ...
    D --> W[去抖 ~300ms 写 draft.json<br/>持久层：OPFS → localStorage 两级]
    W --> PV[底部「预览 · 示例」按草稿渲染]
...
| ~~focusBubblePreset / focusUserBubblePreset~~ | **废弃移除**（决策 #9） | 原 6 套预设下拉被皮肤包导入导出替换；字段从 schema 删除，settings.yaml 残留旧值无害（宿主放行未知键） | — |
...
| 场景 | 数据层行为 | UI 反馈 |
|------|-----------|---------|
| OPFS 不可用（隐私模式/旧浏览器） | 特性检测失败，皮肤 Tab 只读禁用 | 顶部横幅「当前浏览器不支持皮肤库」；半透明/普通背景图功能不受影响 |
...
| 项 | 契约 |
|----|------|
| 物理位置 | OPFS `<专用根目录>/draft.json`；OPFS 不可用降级 localStorage（键 `dsh-chat-focus:draft`）；两级持久层皆不可用则不启用两段式，回落即时提交（M1 §8；不以内存充当缓存文件） |
| 术语边界 | 页面运行期未保存状态 = 组件工作副本（内存、易失），非缓存文件；草稿缓存文件 = draft.json 持久层，二者严格区分 |
| 内容 | `version: 1` + `updatedAt` + `values`（『对话显示』四分组全字段单文件整值快照，含两侧 skinId 与叠加参数） |
...
- 皮肤激活时**替换背景层**：背景色/图片/渐变让位（相关控件禁用并提示），文字色/字体/遮罩/半透明控件继续生效；内容 padding 由皮肤安全区接管；
- **编辑器级 zip 导入导出替换预设功能**（决策 #9）：M3 工作台直接打开/导出 .dshskin；原 6 套 BUBBLE_PRESETS 预设下拉与 focusBubblePreset 字段废弃，内置模板转为随包内置皮肤（首次访问皮肤库时惰性注册，可恢复）；
...
| 输入校验 | 上传/导入在解码层校验魔数（不信任扩展名）；单皮肤资产 ≤20MB、帧数 ≤120、fps ≤24 |
...
### 9.1 设计修订登记（v1.1.0–v1.2.0，待实施，目标里程碑 v0.5.0）
...
| 新增持久化 | — | draft.json（M1 DraftStore；持久层 OPFS→localStorage 两级，皆不可用则回落即时提交，不以内存充当缓存文件） | M1 §8、总规划 §5.4 |
| 新增 UI | 底部固定预览（标题已是「预览 · 示例」，无操作条） | 预览区增加 [保存]/[放弃更改] 操作条 + 未保存标识 + 草稿恢复横幅 | M6 §2 |
